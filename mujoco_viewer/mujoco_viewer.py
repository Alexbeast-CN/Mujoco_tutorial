...
def get_secondary_monitor():
    """获取第二个显示器，如果不存在则返回主显示器"""
    monitors = glfw.get_monitors()
    if len(monitors) > 1:
        return monitors[1]  # 返回第二个显示器
    return glfw.get_primary_monitor()  # 如果只有一个显示器，返回主显示器


def create_window_on_monitor(monitor, width, height, title):
    """在指定显示器上创建窗口"""
    # 获取显示器的工作区域
    x, y, monitor_width, monitor_height = glfw.get_monitor_workarea(monitor)

    # 如果没有指定窗口大小，使用显示器的工作区域大小
    if not width:
        width = monitor_width
    if not height:
...
class MujocoViewer(Callbacks):
    def __init__(
        ...
        target_monitor = get_secondary_monitor()
        self.window = create_window_on_monitor(target_monitor, width, height, title)
        ...
        self.user_scn = mujoco.MjvScene(self.model, maxgeom=10000)
    ...
    def _add_marker_to_scene(self, marker):
        if self.user_scn.ngeom >= self.user_scn.maxgeom:
            raise RuntimeError("Ran out of geoms. maxgeom: %d" % self.user_scn.maxgeom)

        g = self.user_scn.geoms[self.user_scn.ngeom]
        # default values.
        g.dataid = -1
    ...
    def read_pixels(self, camid=None, depth=False):
        ...
        # render
        mujoco.mjr_render(self.viewport, self.user_scn, self.ctx)
    ...
    def render(self):
        ...
        def update():
            ...
            with self._gui_lock:
                ...
                # overlay items
                if not self._hide_menus:  # 只在菜单可见时渲染overlay
